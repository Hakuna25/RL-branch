...
from wiserl.agent.qlearning_agent.qlearning_agent import QLearningAgent
from wiserl.core.wise_rl import WiseRL
import wiserl.agent.qlearning_agent.config as cfg
import gym
import time
import ray
...
class GymRunner(Runner):
    def __init__(self, local_rank=0):
        ...
        if self.rank == 0:
            wise_rl.makeQLAgent(name='ql_agent', agent_class=QLearningAgent, \
            ncol=cfg.ncol, nrow=cfg.nrow, n_action=4, sync=True)
            self.agent = wise_rl.getAgent('ql_agent')
        else:
            self.agent = wise_rl.getAgent('ql_agent')
    ...
    def run(self):
        ...
        plt.xlabel('episode')
        plt.ylabel('return')
        plt.savefig('Qlearning Cliff.png')

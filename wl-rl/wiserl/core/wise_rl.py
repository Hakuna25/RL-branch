...
class WiseRL(object):
    ...
    def makeAgent(self,name,agent_class,net_class,n_states ,n_actions,config=None,num=1, sync=True,use_ray=True) :
        copy_name= None
        copy_agent = None
        if sync == False:
            copy_name="_wise_copy_" + name + str(uuid.uuid1())
            copy_agent =ray.remote(agent_class).remote(net_class,n_states,n_actions,config,sync,use_ray)
            self.registre.addAgent.remote(copy_name,copy_agent,copy_agent)
            copy_agent.setRegistre.remote(self.registre) 
        for i in range(num):
            agent =ray.remote(agent_class).remote(net_class,n_states,n_actions,config,sync,use_ray)
    ...
    def makeACAgent(self, name, agent_class, actor_net, critic_net, n_states, n_actions, config=None, num=1,
                     sync=True):
        copy_name = None
        if sync is not True:
            copy_name = "_wise_copy_" + name + str(uuid.uuid1())
    ...
    def makeA2CAgent(self, name, agent_class, actor_net, n_states, n_actions, config=None, num=1,
                     sync=True):
        copy_name = None
        if sync is not True:
            copy_name = "_wise_copy_" + name + str(uuid.uuid1())
        for i in range(num):
            ...
            if sync is not True:
                agent.setCopyName.remote(copy_name)
                copy_agent = ray.remote(agent_class).remote(actor_net, n_states, n_actions, config,
                                                            sync)
                self.registre.addAgent.remote(copy_name, copy_agent)
                copy_agent.setRegistre.remote(self.registre)
        retref = self.registre.getAllAgent.remote(name)

...
class DoubleDQNAgent(Agent):
    def __init__(self,net_class,n_states,n_actions,config=None, sync=True, use_ray=True):
        super().__init__(sync)
        self.config = cfg
    ...
    def update(self,  s, a, r, s_, dones):
        self.memory_store.push(s, a, r, s_, dones)
        ...
        b_s, b_a, b_r, b_s_, dones = self.memory_store.sample(self.config.BATCH_SIZE, self.n_states)
        b_a = b_a.to(device)
        b_s = b_s.to(device)
        b_r = b_r.to(device)
        b_s_ = b_s_.to(device)
        ...
        max_action = self.eval_net(b_s_).max(1)[1].view(-1, 1)
        max_next_q_values = self.target_net(b_s_).detach().gather(1, max_action) # detach from computational graph, don't back propagate
        # select the maximum q value
        # q_next.max(1) returns the max value along the axis=1 and its corresponding index
        q_target = b_r + self.config.GAMMA * max_next_q_values.view(self.config.BATCH_SIZE, 1)  * (1 - dones)# (batch_size, 1)

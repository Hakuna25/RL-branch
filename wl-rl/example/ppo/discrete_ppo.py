...
from wiserl.net.ppo_net import PolicyNet, ValueNet
...
class GymRunner(Runner):
    def __init__(self, local_rank=0):
        ...
        if use_ray:
            if local_rank == 0:
                 wise_rl.makeACAgent(name="ppo_agent", agent_class=PPOAgent, actor_net=PolicyNet,
                                 critic_net=ValueNet, n_states=N_STATES,n_actions=N_ACTIONS, sync=True)
            self.agent = wise_rl.getAgent("ppo_agent")
        else:
            self.agent = PPOAgent(PolicyNet, ValueNet, N_STATES, N_ACTIONS, config, sync=False, use_ray=use_ray)
    ...
    def run(self):
        ...
        plt.xlabel('Episodes')
        plt.ylabel('Returns')
        plt.title('PPO on {}'.format(self.env_name))
        plt.savefig('../../result/PPO.png')
        plt.show()

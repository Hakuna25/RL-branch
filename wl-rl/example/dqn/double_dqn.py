...
from wiserl.agent.dqn_agent.double_dqn_agent import DoubleDQNAgent
...
class GymRunner(Runner):
    def __init__(self, local_rank=0):
        ...
        if use_ray:
            if local_rank == 0:
                wise_rl.makeAgent(name="double_dqn_agent", agent_class=DoubleDQNAgent, net_class=DQNNet, n_states=N_STATES,
                                n_actions=N_ACTIONS, sync=False, use_ray=use_ray)
                self.agent = wise_rl.getAgent("double_dqn_agent")
            else:
                self.agent = wise_rl.getAgent("double_dqn_agent")
        else:
            self.agent = DoubleDQNAgent(DQNNet,N_STATES,N_ACTIONS, config, sync=False, use_ray=use_ray)
    ...
    def run(self):
        ...
        plt.xlabel('Episodes')
        plt.ylabel('Returns')
        plt.title('DoubleDQN on {}'.format(self.env_name))
        plt.savefig('../../result/DoubleDQN.png')
        plt.show()

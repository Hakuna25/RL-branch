...
from wiserl.agent.ac_agent.ac_agent import ACAgent
from wiserl.net.ac_net import PolicyNet, ValueNet
...
class GymRunner(Runner):
    def __init__(self, local_rank=0):
        ...
        if use_ray:
            if local_rank == 0:
                 wise_rl.makeACAgent(name="ac_agent", agent_class=ACAgent, actor_net=PolicyNet,
                                 critic_net=ValueNet, n_states=N_STATES, n_actions=N_ACTIONS, sync=True)
            self.agent = wise_rl.getAgent("ac_agent")
        else:
            self.agent = ACAgent(PolicyNet, ValueNet, N_STATES, N_ACTIONS, config, sync=False, use_ray=use_ray)
    ...
    def run(self):
        ...
        plt.xlabel('Episodes')
        plt.ylabel('Returns')
        plt.title('AC on {}'.format(self.env_name))
        plt.savefig('../../result/AC.png')
        plt.show()
